...
def skill_executor(skill_name: str, tool_name: str, tool_input: str) -> str:
    ...
    スキルタイプ（local / mcp_runtime / gateway）を判定して実行する。
    ...
    skill_type = registry.get_type(skill_name)

    # mcp_runtime スキル（OAuth 認証が必要なスキル）
    if skill_type == "mcp_runtime":
        from agent.mcp.mcp_runtime_client import MCPRuntimeClient
        scopes = registry.get_scopes(skill_name)
        client = MCPRuntimeClient(provider_name=skill_name, scopes=scopes)
